  ...
  const handlePinSuccess = useCallback(async (
    ...
    try {
      ...
      if (!pinValidation.success) {
        const errorMessage = pinValidation.error || "Invalid PIN. Please try again.";
      ...
      switch (options.transactionType) {
        case 'transfer':
          response = await api.transferMoney(options.transactionData, user.clerkUserId);
          if (response.success) {
            const symbol = options.transactionData.accountCurrency === "EUR" ? "€" :
        ...
        case 'withdraw':
          response = await api.withdrawMoney(options.transactionData, user.clerkUserId);
          if (response.success) {
            const symbol = options.transactionData.accountCurrency === "EUR" ? "€" :
                          options.transactionData.accountCurrency === "GBP" ? "£" : "$";
        ...
        case 'deposit':
          response = await api.depositMoney(options.transactionData, user.clerkUserId);
          if (response.success) {
            const symbol = options.transactionData.accountCurrency === "EUR" ? "€" :
                          options.transactionData.accountCurrency === "GBP" ? "£" : "$";
        ...
        case 'convert':
          response = await api.convertMoney(options.transactionData, user.clerkUserId);
          if (response.success) {
            const fromSymbol = options.transactionData.convertFromAccountCurrency === "EUR" ? "€" :
                               options.transactionData.convertFromAccountCurrency === "GBP" ? "£" : "$";
            const toSymbol = options.transactionData.convertToAccountCurrency === "EUR" ? "€" :
